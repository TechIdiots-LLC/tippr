...
###############################################################################
"""Pure Python fallback for sgm.pyx (used when Cython module is not compiled).

SGM = Smart Get Multi - a cache-aside pattern implementation that fetches from
cache first, then calls miss_fn for any missing keys and caches the results.
"""

from r2.lib.cache import MemcachedError


def sgm(cache, keys, miss_fn, prefix='', time=0, stale=False,
        found_fn=None, _update=False, stat_subname=None,
        ignore_set_errors=False):
    """Smart get multi: fetch from cache, compute misses, cache results.
    ...
    Args:
        cache: Cache object with get_multi/set_multi methods
        keys: Iterable of keys to fetch
        miss_fn: Function called with set of missing keys, returns dict of results
        prefix: Prefix for cache keys
        time: TTL for cached entries
        stale: Whether to accept stale cache entries
        found_fn: Optional callback(ret, still_need) to filter cache hits
        _update: If True, skip cache lookup and compute all keys
        stat_subname: Subname for cache statistics
        ignore_set_errors: If True, ignore MemcachedError on set_multi

    Returns:
        Dict mapping keys to values (from cache or miss_fn)
    """
    ret = {}

    # map the string versions of the keys to the real version. we only
    # need this to interpret the cache's response and turn it back
    ...
    if _update:
        ...
        cached = cache.get_multi(list(s_keys.keys()), prefix=prefix, **kw)
